...
CREATE POLICY "Users can manage contact tags" ON contact_tags FOR ALL USING (
    EXISTS (
        ...
        FROM contacts
        WHERE
            contacts.id = contact_tags.contact_id
...
CREATE POLICY "Users can manage custom values" ON contact_custom_values FOR ALL USING (
    EXISTS (
        SELECT 1
        FROM contacts
        WHERE
            contacts.id = contact_custom_values.contact_id
            AND contacts.user_id = auth.uid ()
    )
);
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    sender_type TEXT NOT NULL CHECK (
        sender_type IN ('customer', 'agent', 'bot')
    ),
    sender_id UUID,
...
CREATE POLICY "Service role can insert messages" ON messages FOR
INSERT
WITH
    CHECK (true);
...
CREATE TABLE IF NOT EXISTS message_templates (
    ...
    user_id UUID NOT NULL REFERENCES auth.users (id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    category TEXT NOT NULL DEFAULT 'Marketing' CHECK (
        category IN (
            'Marketing',
            'Utility',
            'Authentication'
...
CREATE POLICY "Users can manage broadcast recipients" ON broadcast_recipients FOR ALL USING (
    EXISTS (
        SELECT 1
        FROM broadcasts
        WHERE
            broadcasts.id = broadcast_recipients.broadcast_id
